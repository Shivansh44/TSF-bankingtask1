...
mongoose.connect("mongodb+srv://database:Ashish123@cluster0.sipku.mongodb.net/Bank?retryWrites=true&w=majority" || "mongodb://localhost:27017/banking", {
...
app.post("/customer", wrapAsync(async(req, res, next) => {
    ...
    const cus = await Customer.findOne({
        username: username
    }); //using findOne Instead Of find so that it return an object not array
    const b = await Customer.findOne({
        username: From
    });
    if (!cus || !b) {
        throw new AppError("User Not Found", 401);
...
app.use((err, req, res, next) => {
    const {
        status = 401, message = "Something wrong Occured"
    } = err;
    if (!err.message)
...
app.listen(PORT, (req, res) => {
    console.log("Your Bank is always ready serving you!!")
});